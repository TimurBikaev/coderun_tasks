...
public class Sorting_bubbles {
    ...
    public static void main(String[] args) {
        ...
        //изначально принимаем, что массив неотсортирован
        boolean isSorted = false;
        System.out.println();
        System.out.println("Исходный массив: " + Arrays.toString(array)); //переводим массив в строку для вывода (иначе выведет хэш)

        int iteration = 0; // счетчик итераций

        //Внешний цикл, который продолжается, пока массив не отсортирован
        while (!isSorted) { //цикл while всегда выполняется полностью как минимум 1 раз,
                            // таким образом дойдя до конца цикла isSorted становился false,
                            // до тех пор пока if не пропустится и isSorted останется true.
                            // тогда по условию !true внешний цикл не станет выполняться
            isSorted = true;
            //Как только весь массив будет отсортирован (то есть ни одного обмена не произойдет), isSorted останется true, и цикл завершится.
            //перебираем элементы
            for (int i = 1; i < n; i++) {
                //сравниваем каждую пару
                if (array[i] < array[i - 1]) {
                    // Меняем их местами, если порядок неправильный
                    int temp = array[i]; //чтобы не перезатереть элемент, сохраним его во временную переменную
                    array[i] = array[i - 1];
                    array[i - 1] = temp; //устанавливаем на место первой переменной [i-1] записываем вторую [i]
                    //если дошли сюда, значит сортировка произошла, сбрасываем флаг к false, иначе while остановится
                    isSorted = false;
                }
            }
            n--;  // Уменьшаем область сравнения
            System.out.println("Область сравнения составляет " + n + " символов");
            iteration++; // Прибавляем счетчик итераций
